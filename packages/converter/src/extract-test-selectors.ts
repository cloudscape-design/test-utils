...
const srcTestUtilsPath = 'src/test-utils';
...
export function extractTestSelectorsUtil() {
  ...
  for (const [filePath, properties] of Object.entries(selectorsFilePathToUsedProperties)) {
...
function getComponentNameFromFilePath(filePath: string) {
  return filePath.match(componentPathRegEx)![1];
}
